...
$(function() {
  ...
  $('a[data-button="ppUpdateButton"]').click(function(e) {
    ...
    if ( OSCOM.APP.PAYPAL.getUpdatesProgress == 'retrieve' ) {
      ...
      window.location('<?php echo OSCOM::link('admin/apps.php', 'PayPal&action=update'); ?>');
  ...
  OSCOM.APP.PAYPAL.doUpdate = function() {
    ...
    if ( versions.length > 0 ) {
      var runQueueInOrder = function(i) {
        ...
        $.getJSON('<?php echo OSCOM::link('admin/apps.php', 'PayPal&action=update&subaction=download&v=APPDLV'); ?>'.replace('APPDLV', versions[i]), function (data) {
          if ( (typeof data == 'object') && ('rpcStatus' in data) && (data['rpcStatus'] == 1) ) {
            $('#ppUpdateInfo div').append('<p>' + def['applying_version_progress'].replace(':version', OSCOM.htmlSpecialChars(versions[i])) + '</p>');

            $.getJSON('<?php echo OSCOM::link('admin/apps.php', 'PayPal&action=update&subaction=apply&v=APPDLV'); ?>'.replace('APPDLV', versions[i]), function (data) {
              if ( (typeof data == 'object') && ('rpcStatus' in data) && (data['rpcStatus'] == 1) ) {
              } else {
                updateError = true;
                ...
                $.getJSON('<?php echo OSCOM::link('admin/apps.php', 'PayPal&action=update&subaction=log&v=APPDLV'); ?>'.replace('APPDLV', versions[i]), function (data) {
                  if ( (typeof data == 'object') && ('rpcStatus' in data) && (data['rpcStatus'] == 1) ) {
