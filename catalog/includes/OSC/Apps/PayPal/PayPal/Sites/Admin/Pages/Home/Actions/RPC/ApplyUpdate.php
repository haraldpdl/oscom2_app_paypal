...
class ApplyUpdate extends \OSC\OM\PagesActionsAbstract
{
    public function execute()
    {
        ...
        if (isset($_GET['v']) && is_numeric($_GET['v']) && ($_GET['v'] > $OSCOM_PayPal->getVersion())) {
            ...
            $update_zip = OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/work/update.zip';

// reset the log
            if (file_exists(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/work/update_log-' . $update_version . '.php') && is_writable(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/work/update_log-' . $update_version . '.php')) {
                unlink(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/work/update_log-' . $update_version . '.php');
            }

            if (file_exists($update_zip)) {
                $work_dir = OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/work/update_contents';
                ...
                if (file_exists($update_pkg) && file_exists($update_pkg . '.sig')) {
                    $public = openssl_get_publickey(file_get_contents(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/work/paypal.pubkey'));
                    ...
                    if (openssl_verify(sha1_file($update_pkg), file_get_contents($update_pkg . '.sig'), $public) === 1) {
                        ...
                        if ($meta_pass === true) {
                            ...
                            if (!$OSCOM_PayPal->isWritable(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/version.txt')) {
                                $errors[] = $OSCOM_PayPal->displayPath(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/version.txt');
                            }
                            ...
                            if (empty($errors)) {
                                $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_update_started', [
                                    'version' => $update_version
                                ]), $update_version);
                                    ...
                                    if (substr($pathname, 0, 8) == 'catalog/') {
                                        ...
                                        if (copy($file, DIR_FS_CATALOG . $target . basename($pathname))) {
                                            $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_updated_file', [
                                                'filepath' => $OSCOM_PayPal->displayPath(DIR_FS_CATALOG . $target . basename($pathname))
                                            ]), $update_version);
                                        } else {
                                        ...
                                        if (copy($file, DIR_FS_ADMIN . $target . basename($pathname))) {
                                            $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_updated_file', [
                                                'filepath' => $OSCOM_PayPal->displayPath(DIR_FS_ADMIN . $target . basename($pathname))
                                            ]), $update_version);
                                        } else {
                                ...
                                if (empty($errors)) {
                                    if (file_put_contents(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/version.txt', $update_version)) {
                                        $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_updated_file', [
                                            'filepath' => $OSCOM_PayPal->displayPath(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/version.txt')
                                        ]), $update_version);
                                    } else {
                                        $errors[] = $OSCOM_PayPal->displayPath(OSCOM::BASE_DIR . 'OSC/Apps/PayPal/PayPal/version.txt');
                                    }
                                ...
                                if (empty($errors)) {
                                    $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_update_success', [
                                        'version' => $update_version
                                    ]), $update_version);

                                    $result['rpcStatus'] = 1;
                                } else {
                                    $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_update_failed', [
                                        'version' => $update_version
                                    ]), $update_version);
                                }
                            }
                ...
                $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('error_log_nonexisting', [
                    'filepath' => $update_zip
                ]), $update_version);
            }
        }
